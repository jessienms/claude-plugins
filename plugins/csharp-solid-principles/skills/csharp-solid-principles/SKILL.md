...
이 스킬은 **두 가지 모드**로 동작한다. 먼저 어느 모드인지 판단하고 그에 맞게 실행하라.

### 모드 A — 리뷰 모드 (기존 코드 점검)

사용자가 **이미 있는 코드**를 SOLID 관점에서 점검·리뷰·평가해 달라고 할 때
(예: "이 클래스 SOLID 점검해줘", "이거 너무 많은 일 하는 거 아냐?", "설계 리뷰해줘").

이때는 **반드시 새 서브에이전트를 띄워 그 안에서 리뷰를 수행**한다. 현재 대화창(메인 세션)에서 직접 리뷰하지 마라.

이유: 메인 세션에는 해당 코드를 방금 작성했거나 옹호하던 문맥이 남아 있어 리뷰가 관대해진다(작성자 = 리뷰어 편향). 대화 이력이 없는 깨끗한 문맥의 서브에이전트가 코드만 보고 판단해야 결과가 객관적이다.

**실행 절차:**

1. 리뷰 대상(파일 경로 또는 코드 범위)을 특정한다.
2. `Agent` 툴로 `general-purpose` 서브에이전트를 **하나** 띄운다. 아래 프롬프트 템플릿을 그대로 전달한다.
3. 서브에이전트가 반환한 리포트를 사용자에게 그대로 정리해 전달한다. 메인 세션은 리뷰 판단을 새로 하지 않는다.

**서브에이전트에 전달할 프롬프트 템플릿:**

```
너는 독립적인 C# 코드 리뷰어다. 아래 대상 코드를 SOLID 원칙 기준으로만 냉정하게 평가하라.
이 코드를 누가 왜 작성했는지에 대한 배경은 없으며, 오직 코드 자체로만 판단한다.

대상: <파일 경로 / 코드 범위>

지시:
- 대상 파일을 직접 읽어라(Read). 필요하면 관련 타입도 따라가 읽어라.
- SKILL.md 의 "SOLID 리뷰 체크리스트"(SRP/OCP/LSP/ISP/DIP)를 각각 적용하라.
- 각 위반은 [원칙] · [위치 file:line] · [무엇이 왜 문제인지] · [권장 리팩터링] 형식으로 보고하라.
- 위반이 없으면 "위반 없음"이라고 명확히 말하라. 억지로 문제를 만들지 마라.
- 심각도(높음/중간/낮음)를 매겨 정렬하라.
- **다섯 원칙 각각을 별점 5점 만점으로 평가하라**(아래 별점 기준 참고). 근거를 한 줄로 덧붙여라.
- 코드를 수정하지 마라. 오직 리포트만 반환한다.
...
> 참고: 읽기 전용 관점을 더 강하게 강제하고 싶으면 `Explore` 에이전트를 쓸 수 있으나, 실제 위반 판정·리포트 작성 품질은 `general-purpose` 가 낫다. 어느 쪽이든 **서브에이전트는 코드를 수정하지 않고 리포트만 반환**해야 한다.

### 모드 B — 참고 모드 (코드 작성 중)

사용자가 코드를 **작성·리팩터링하는 도중** 원칙을 물어보거나 예시를 요구할 때
(예: "SRP가 뭐야?", "이 인터페이스 어떻게 나눠?", "테스트 가능하게 만들려면?").

이때는 서브에이전트를 띄우지 말고, 아래 참조 내용을 **인라인으로** 활용해 바로 답한다.
