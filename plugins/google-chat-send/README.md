...
webhook(비밀)과 전송 이력이 저장되는 폴더는 아래 순서로 결정됩니다.

1. `-DataDir <경로>`를 직접 주면 그대로 사용합니다.
2. 프로젝트 폴더(`$CLAUDE_PROJECT_DIR`, 없으면 현재 작업 폴더)에 `.claude/google-chat/`이 **이미 있으면** 그대로 사용합니다.
3. git 저장소라면 **메인 체크아웃**의 `.claude/google-chat/`을 사용합니다.
4. 그 외(git 저장소가 아니거나 git이 없을 때)는 프로젝트 폴더 아래 `.claude/google-chat/`.

3번 덕분에 **git worktree나 저장소 하위 폴더에서 작업해도 메인 체크아웃에 등록해 둔 webhook을 그대로 사용합니다.** worktree마다 webhook을 다시 등록하거나 `-DataDir`을 손으로 붙일 필요가 없습니다.

지금 어느 폴더를 쓰고 있고 왜 그렇게 정해졌는지는 "구글챗 설정 상태 알려줘"로 확인할 수 있습니다(`init.ps1`의 `경로 결정 규칙` 줄).
