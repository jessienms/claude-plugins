...
- **데이터 폴더는 자동으로 정해집니다.** Codex에는 `CLAUDE_PROJECT_DIR` 같은 프로젝트 루트 변수가
  없어 **현재 작업 폴더**가 기준이 되지만, 그 폴더가 git 저장소 안이면 스크립트가
  **메인 체크아웃**의 `.claude/google-chat/`을 찾아 씁니다. 즉 저장소 하위 폴더나 git worktree에서
  실행해도 이미 등록해 둔 webhook을 그대로 사용하며, `-DataDir`을 손으로 붙일 필요가 없습니다.
  (결정 순서: `-DataDir` → 현재 폴더의 기존 데이터 폴더 → git 메인 체크아웃 → 현재 폴더 기본값.
  지금 어느 폴더가 왜 선택됐는지는 "구글챗 설정 상태 알려줘"로 확인할 수 있습니다.)
