...
목표는 **작성자 = 리뷰어 편향**을 제거하는 것이다. 방금 그 코드를 작성했거나 옹호하던 맥락이 남아 있으면 리뷰가 관대해진다. Codex에는 별도의 서브에이전트로 리뷰를 위임하는 툴이 없으므로, 아래 **독립 리뷰어 규율**로 이 편향을 억제한다.

**독립 리뷰어 규율:**

- 너 자신을 이 코드와 아무 관련 없는 **외부 리뷰어**로 간주하라. 이전 대화에서 이 코드를 네가 작성했거나 설계를 옹호했더라도, 그 맥락과 애착을 버리고 **코드 그 자체만** 근거로 판단한다.
- 대상 파일을 **지금 다시 읽어라**(직전 기억이나 요약에 의존하지 말 것). 필요하면 관련 타입·인터페이스도 따라가 읽는다.
- "아마 의도가 있었을 것"이라며 넘어가지 마라. 계약(contract)과 코드가 실제로 보장하는 것만 본다.

> **가장 객관적인 결과가 필요하면:** 이 리뷰를 **작성 맥락이 전혀 없는 새 Codex 세션**에서 실행하라(`$csharp-solid-principles` 로 명시 호출하거나, 새 세션에서 "이 파일 SOLID 리뷰해줘"라고 요청). 새 세션에는 옹호할 이력이 없어 판정이 더 엄격해진다.

**실행 절차:**

1. 리뷰 대상(파일 경로 또는 코드 범위)을 특정한다.
2. 대상 파일을 직접 읽는다. 필요하면 관련 타입도 읽는다.
3. 아래 "SOLID 리뷰 체크리스트"(SRP/OCP/LSP/ISP/DIP)를 각각 적용한다.
4. 각 위반은 `[원칙] · [위치 file:line] · [무엇이 왜 문제인지] · [권장 리팩터링]` 형식으로 보고한다.
5. 위반이 없으면 "위반 없음"이라고 명확히 말한다. 억지로 문제를 만들지 마라.
6. 심각도(높음/중간/낮음)를 매겨 정렬한다.
7. **다섯 원칙 각각을 별점 5점 만점으로 평가**한다(아래 별점 기준 참고). 근거를 한 줄로 덧붙인다.
8. 코드를 수정하지 마라. 오직 리포트만 반환한다.
...
이때는 리뷰어 규율을 적용하지 말고, 아래 참조 내용을 **인라인으로** 활용해 바로 답한다.
